...
function t702_initPopup(recId){var rec=document.getElementById('rec'+recId);if(!rec)return;var container=rec.querySelector('.t702');if(!container)return;rec.setAttribute('data-animationappear','off');rec.setAttribute('data-popup-subscribe-inited','y');rec.style.opacity=1;var documentBody=document.body;var popup=rec.querySelector('.t-popup');var popupTooltipHook=popup.getAttribute('data-tooltip-hook');var analitics=popup.getAttribute('data-track-popup');var popupCloseBtn=popup.querySelector('.t-popup__close');var hrefs=rec.querySelectorAll('a[href*="#"]');var submitHref=rec.querySelector('.t-submit[href*="#"]');if(popupTooltipHook){var allTriggerButtons=document.querySelectorAll('a[href="'+popupTooltipHook+'"]');Array.prototype.forEach.call(allTriggerButtons,function(button){if(button){button.setAttribute('role','button');button.setAttribute('aria-haspopup','dialog')}});document.addEventListener('click',function(){var target=event.target;var link=target.closest('a[href*="#"]')?target.closest('a[href*="#"]'):!1;if(!link)return;var arrHref=link.href.split('#');if(decodeURI('#'+arrHref[1])!==popupTooltipHook)return;event.preventDefault();t702_showPopup(recId);t702_resizePopup(recId);t702__lazyLoad();if(analitics){Tilda.sendEventToStatistics(analitics,popupTooltipHook)}})}
