...
class Data_base_connect():
    ...
    def update_mp(self, shop_name, mp, key):
        self.conn.autocommit=True
        self.curs = self.conn.cursor()
        self.curs.execute(f"UPDATE marketplaces SET key_mp='{key}' "
                            f" WHERE shop_name='{shop_name}' AND name_mp='{mp}'")
        self.curs.close()
        # pass
