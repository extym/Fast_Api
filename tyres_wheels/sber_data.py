...
from categories import categories_wheels, cats_wheels_upper
...
for key in categories_wheels.keys():
    category_vendor = key
    categories_id = categories_wheels[category_vendor]
    categoryChild = root.createElement('category')
...
with open(DATA_IMG + "yandex.xml", "w") as f:
    f.write(xml_str)

with open(DATA_IMG + "sber.xml", "w") as f:
    f.write(xml_str)
