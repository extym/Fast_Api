...
# from connect import write  #TODO Now it's mandatory gor production uncomment
...
list_vendors = ['iFree', 'Carwel', 'KHOMEN', 'КиК', 'Скад']

def write(smth):
    try:
        with open('log.txt', 'a') as file:
            how_time = datetime.now()
...
def get_need_data():
    ...
    with open('data_product.json', 'r') as file:
        ...
        try:
            ...
            # TODO Now it's mandatory gor production uncomment
...
def get_need_data_v2():
    ...
    with open('data_product.json', 'r') as file:
        ...
        try:
            for lin in our_data:
                quantity = int(lin.get('rest').strip('"').strip('>'))
                category = lin.get('type').strip('"')
                if quantity < 4 or category not in ["Штампованные диски", "Грузовые диски", "Литые диски"]:   # "Шины грузовые"
                    # print(category)
                    continue
                else:
                    price = lin.get('price').strip('"').replace('\xa0', '').split('.')[0]
                    rrc = lin.get('RoznicaPrice').strip('"').strip().replace('\xa0', '').split('.')[0]
                    try:
                        rule = int(rrc) / int(price)

                    except:
                        print(12345, rrc, price, lin)
                        continue
                    if rule >= 0.18 and int(price) >= 7000:
                        need_data.append(lin)
    ...
    print('need_data2 - ', len(need_data), need_data[-3])
    with open("need_data.json", "w") as write_file:
        json.dump(need_data, write_file) # encode dict into JSON
