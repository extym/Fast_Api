...
async def avito_webhook():
    ...
    if request.headers.get('X-Avito-Messenger-Signature'):
        ...
        logging.info('WE_GET_HOOK_from_avito' + str(hook))
        ...
        try:
            ...
            if author_id in sender_ids and msg == bot_answer:
                chat_id = hook.get('payload').get('value').get('chat_id')
            ...
            ## random message, not first, not our
                ...
                try:
                    # logging.info('from_webhook_before_get_chat {} {} {} {}'.format
                    #              (check, msg_id, chat_id, user_id))
...
async def wait_amo_hook():
    if request.method == "POST":
        ...
        logging.info('WE_GET_DATA_FROM_AMO' + str(data_json))
        try:
            # make_data_for_avito(data_json)
            await make_data_for_avito_v2(data_json)
            # logging.info('WE_SEND_DATA_TO_AVITO' + str(data_json))
        except Exception as err:
            logging.error('WHAT_is_FUCK_UP_error from endpoint location {}'.format(err))
            send_get('WHAT_is_FUCK_UP_error from endpoint location {}'.format(err))
...
async def bonus_hook():
    if request.method == 'POST':
        ...
        await make_bonus(subdomain, lead_id)
        logging.info('Make_bonus', subdomain, lead_id)
        # except:
        #     print(55555555555, request.get_data(as_text=True))
        response = app.response_class(
            status=200
        )
    else:
