...
def get_data_for_cards_from_xml(category_name: str = None,
        ...
        for row in doc['yml_catalog']['shop']['offers']['offer']:
            ...
            if name and category_name in name:
                ...
                prod = {
                    ...
                    "Ссылка на pdf-инструкцию": row.get("Ссылка на pdf-инструкцию"),
                    "Ссылка на сертификат": row.get("Ссылка на сертификат"),
                    "Вес упаковки (кг)*": row.get("Вес упаковки (кг)*"),
                    "Высота упаковки (см)*": 12,
                    "Ширина упаковки (см)*": 15,
                    "Длина упаковки (см)*": 28,
                    "OEM": row.get("OEM"),
                    "Маркетинговый цвет": row.get("Маркетинговый цвет"),
                    "Примечание": row.get("Примечание"),
                    "Цена за": row.get("Цена за"),
                    "CROSS VENDOR": row.get("CROSS VENDOR"),
                    "CROSS VENDOR COD": row.get("CROSS VENDOR COD"),
                    "Количество в упаковке, в штуках": row.get("Количество в упаковке, в штуках"),
    ...
    with pd.ExcelWriter('card-12345.xlsx', mode='a', if_sheet_exists='overlay') as writer:
        pr.to_excel(writer, sheet_name='Монтажный комплект тормозных ко', startrow=3, index=False, header=False)

    # return brand, oem
    # print(456, len(header))
