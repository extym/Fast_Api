...
url_init = "http://46.173.219.89:8888/"  #'http://localhost:8888/'  #'https://api.pay2.pro/' + metod
...
# пункт 1 из мануала по подписи данных -
# имя вызываемого скрипта (от последнего '/' до конца или '?')
metod = "init_payment"
# пункт 2 из мануала по подписи данных
# все поля сообщения (начинаются с префикса sp_), отсортированные в
# алфавитном порядке, включая случайную строку sp_salt
target_string = metod  + ';' + str(sp_amount) + ';' + sp_currency + ';' + sp_description + ';' + str(sp_outlet_id) + ';' \
                + sp_payment_system + ';' + sp_salt + ';' + sp_user_contact_email + ';' + sp_user_ip + ';' \
                + sp_user_name + ';' + sp_user_phone + ';' + secret_key  # пункт 3 из мануала по подписи данных - платежный пароль secret_key
# Подпись
