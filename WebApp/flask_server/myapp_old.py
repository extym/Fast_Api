...
def index():
    ...
    return unescape(render_template("index.html", url=url, login=LOGIN, passw=PASSW, status=status,
                                    sellers_today_href=sellers_today_href,
...
def index_old():
    ...
    db = MsDatabase(host=MYSQL_HOST, user=MYSQL_USER, passwd=MYSQL_PASW, database=MYSQL_DATABASE)
    url = url_for('index_old', _external=True)
    url_today = url_for('sellers_today', _external=True)
    dashboard_url = url_for("dashboard", _external=True)
    seller_orders_url = url_for('seller_orders', _external=True)
    seller_acts_url = url_for('seller_acts', _external=True)
    ...
    #not_ass_labels = f"<a href='{URL}labels_not_assembled/all_labels.pdf' target='_blank'>Все этикетки</a>"
    #not_ass_labels_day = f"<a href='{URL}labels_not_assembled_day/all_labels_day.pdf' target='_blank'>Этикетки сегодня</a>"
    return unescape(render_template("index_old.html", url=url, login=LOGIN, passw=PASSW, sellers=sellers, status=status,
                                    contragents=contragents, sellers_today_href=sellers_today_href, dashboard_href=dashboard_href))
...
def keys():
    ...
    db = MsDatabase(host=MYSQL_HOST, user=MYSQL_USER, passwd=MYSQL_PASW, database=MYSQL_DATABASE)
    url = url_for('index', _external=True)
    keys_url = url_for('keys', _external=True)

    return unescape(render_template('keys.html', keys_url=keys_url, url=url))


@app.route("/dashboard/menage_users", methods=['GET', 'POST'])
def manage_users():
    if LOGGING:
        logging.basicConfig(filename=os.path.join(PUBLIC_DIR, LOG_FILE),
                            format='[%(asctime)s] [%(levelname)s] => %(message)s',
    ...
    db = MsDatabase(host=MYSQL_HOST, user=MYSQL_USER, passwd=MYSQL_PASW, database=MYSQL_DATABASE)
    url = url_for('index', _external=True)
    menage_users_url = url_for('manage_users', _external=True)

    return unescape(render_template('manage-users.html', keys_url=menage_users_url, url=url))
...
###########################################################################
            ...
            for sklad_id in sklads_dict:
                ...
                if act_id is None:
                    ...
                    if not db.insert_act(act_id, seller[3]):
                        status_text += f'Ошибка записи act_id={act_id} и seller_id={seller[3]} в БД<br>'
                        result_status_text += status_text + '\n'
                    else:
    ...
    return unescape(
        render_template("today.html", url_today=sellers_today_href, url=url, login=LOGIN, passw=PASSW, sellers=sellers,
            ...
            dashboard_href=dashboard_href, dashboard_url=dashboard_url ))  #contragents=contragents, name=name, rows=rows,
            ...
            for sklad_id in sklads_dict:
                ...
                if act_id is None:
                    ...
                    if not db.insert_act(act_id, seller_id):
                        status_text += f'Ошибка записи act_id={act_id} и seller_id={seller_id} в БД<br>'
                    else:
                        status_text += f"Акт под номером {act_id} для склада {sklad_id} заказан. Ожидайте pdf.<br>"
        elif 'create_demand' in result:
            # ms_demand: 0 - по умолчанию, 1 - не собран, 2 - не отгружен, 3 - отгружен
                ...
                if order_dict[MS_FIELD_SOBRAN_BOOL] and not order_dict['demands']:
                    ...
                    if demand_dict:
                        if ms.post_demand_v2(demand_dict):
                            ...
                            status_text += f"<br>{order_dict['name']} - ОШИБКА ОТГРУЗКИ!"
