...
def call_sync(request):
    if request.method == "GET":
        response = requests.get(request.host + request.url, headers=request.header, timeout=10)
        limits = get_limits_usage(response)
        json_wrapper = parse_json_from_string(response.text)
        check_response(json_wrapper)
        return (request.json_parser(json_wrapper),limits, response.text)
    elif request.method == "POST":
        response = requests.post(request.host + request.url, headers=request.header, timeout=10)
        limits = get_limits_usage(response)
        json_wrapper = parse_json_from_string(response.text)
        #print(response.text)
        check_response(json_wrapper)
        ...
        response = requests.delete(request.host + request.url, headers=request.header, timeout=10)
        limits = get_limits_usage(response)
        json_wrapper = parse_json_from_string(response.text)
        check_response(json_wrapper)
        return (request.json_parser(json_wrapper),limits, response.text)
    elif request.method == "PUT":
        response = requests.put(request.host + request.url, headers=request.header, timeout=10)
        limits = get_limits_usage(response)
        json_wrapper = parse_json_from_string(response.text)
        check_response(json_wrapper)
        return (request.json_parser(json_wrapper),limits, response.text)
