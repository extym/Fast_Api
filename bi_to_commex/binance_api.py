...
class Binance():
    methods = {
        ...
        'withdraw': {'url': 'sapi/v1/capital/withdraw/apply', 'method': 'POST', 'private': True},
        'depositHistory': {'url': 'sapi/v1/capital/deposit/hisrec', 'method': 'GET', 'private': True},
        ...
        'futuresTransfer': {'url': 'sapi/v1/futures/transfer', 'method': 'POST', 'private': True},
    ...
    def call_api(self, **kwargs):
        ...
        if self.methods[command]['method'] == 'GET' or (self.methods[command]['url'].startswith('sapi') and command != "withdraw" and command != "futuresTransfer"):
            api_url += '?' + payload_str

        response = requests.request(method=self.methods[command]['method'], url=api_url, data="" if self.methods[command]['method'] == 'GET' else payload_str, headers=headers, timeout=10)
